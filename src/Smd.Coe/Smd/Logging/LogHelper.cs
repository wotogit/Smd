...
namespace Smd.Logging
{
    ...
    public static class LogHelper
    {
        ...
        static LogHelper()
        {
            var lf = EngineContext.Current.Resolve<ILoggerFactory>();
            Logger = lf == null
                ? EngineContext.Current.Resolve<ILoggerFactory>().Create(typeof(LogHelper))
                : NullLogger.Instance;
        }
        ...
        private static void LogValidationErrors(ILogger logger, Exception exception)
        {
            ...
            if (exception is AggregateException && exception.InnerException != null)
            {
                ...
                if (aggException.InnerException is SmdValidationException)
                {
                    exception = aggException.InnerException;
                }
            }

            if (!(exception is SmdValidationException))
            {
                return;
            }

            var validationException = exception as SmdValidationException;
