...
{
    public class SmdRoleStore<TRole,TUser> :
        IRoleStore<TRole>,
        ...
        where TUser : SmdUser<TUser>
    {
       ...
       // private readonly IUnitOfWorkManager _unitOfWorkManager;
        ...
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
        /// <returns>A <see cref="Task{TResult}"/> that result of the look up.</returns>
        public virtual Task<TRole> FindByNameAsync([NotNull] string name, CancellationToken cancellationToken = default(CancellationToken))
        {
          //  cancellationToken.ThrowIfCancellationRequested();

          //  Check.NotNull(normalizedName, nameof(normalizedName));

            return _roleRepository.FirstOrDefaultAsync(r => r.Name == name);
        }
